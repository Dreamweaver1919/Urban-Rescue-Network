    ...
    def _nearest_task_strategy(self):
        ...
        for task in self.tasks:
            ...
            while len(current) > 1:
                ...
                need = task.survivors_not_rescued
                total_eff = sum(r.efficiency for r in rest)
                if total_eff * time_left >= need:
        ...
        for r in self.rescuers:
            if not r.busy and available:
                nearest = min(available, key=lambda t: abs(r.x - t.x) + abs(r.y - t.y))
                actions[r.rescuer_id] = nearest.task_id
        return actions
