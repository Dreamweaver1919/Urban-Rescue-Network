...
class RescueEnvCore:
    ...
    def _nearest_task_strategy_Allocation(self):
        ...
        # 步骤4：按task_priority顺序分配救援人员
        assigned_rescuers = set()
        for _, _, task, _ in task_priority:
            ...
            for cand in candidates:
                if assigned >= needed:
                    break
                _, _, rescuer = cand
                if rescuer.rescuer_id not in assigned_rescuers:
                    actions[rescuer.rescuer_id] = task.task_id
                    assigned_rescuers.add(rescuer.rescuer_id)
                    assigned += 1
                    # 从free_rescuers_sorted中移除已分配的救援人员
    ...
    def _nearest_task_strategy_Evacuation(self):
        ...
        for task in available_tasks:
            ...
            for r in free_rescuers:
                ...
                if distance == float('inf'):
                    continue
                arrival_time = distance / r.speed
                if arrival_time < required_time:
                    required_time = arrival_time
            ...
            time_left = task.deadline - self.current_time
            if required_time >= time_left:
                continue  # 跳过无法按时完成的任务

            # 计算紧急度（值越大越紧急）
            urgency = task.survivors_not_rescued / (time_left - required_time + 1e-5)
            task_priority.append((-urgency, task))  # 使用负值便于排序
        ...
        for _, task in task_priority:
            ...
            for r in free_rescuers:
                if r.rescuer_id in assigned_rescuers:
                    continue
            ...
            if closest_rescuer:
                actions[closest_rescuer.rescuer_id] = task.task_id
                assigned_rescuers.add(closest_rescuer.rescuer_id)
                closest_rescuer.move_to_task(task.node_id)
                free_rescuers.remove(closest_rescuer)
...
def Visualize(env):
    ...
    ax_progress_right = ax_progress.twinx()
    ax_progress_right.set_ylabel("Active Tasks / Idle Rescuers", color='teal')
    progress_line2, = ax_progress_right.plot([], [], 'g--', lw=1, label='active tasks')
    progress_line3, = ax_progress_right.plot([], [], 'm:', lw=1, label='idle rescuers')
    ...
    # ================== 动态元素初始化 ==================
    # 生成救援人员颜色映射
    n_rescuers = len(env.rescuers)
    cmap = plt.get_cmap('tab20')
    rescuer_colors = [cmap(i % 20) for i in range(n_rescuers)]  # 预生成颜色列表

    # 初始化救援人员散点图（使用虚拟坐标）
    init_x = [-100] * n_rescuers  # 初始在屏幕外
    init_y = [-100] * n_rescuers
    rescuer_capacities = [r.rescue_capacity for r in env.rescuers]
    sizes = [c * 40 for c in rescuer_capacities]  # 40为缩放因子，可按需调整
    ...
    # ================== 进度图初始化 ==================
    progress_line, = ax_progress.plot([], [], 'b-', lw=2, label='rescued victims')
    ...
    ax_main.set(
        ...
        xlabel="X",
        ylabel="Y",
        title="Real time Rescue Network"
    )
    ...
    ax_progress.legend([progress_line, progress_line2, progress_line3],
                       ['Rescued Victims', 'Active Tasks', 'Idle Rescuers'],
                       loc='upper left')
    ax_progress_right.spines['right'].set_color('teal')
    ...
    time_text = ax_main.text(
        ...
        zorder=100,
        bbox=dict(
            facecolor=(1, 1, 1, 0.3),  # 半透明白色背景（如需完全透明可删掉这行）
            edgecolor='none',
            boxstyle='round',
        )
    )
    ...
    def update(frame):
        ...
        for i, r in enumerate(env.rescuers):
            if r.path:
                path_nodes = [r._get_current_node()] + r.path
                x_coords = []
                y_coords = []
                for n in path_nodes:
                    node = env.graph.nodes[n]
                    x_coords.append(node.x)
                    y_coords.append(node.y)
                path_lines[i].set_data(x_coords, y_coords)
            else:
        ...
        rescuer_pos = np.array(rescuer_pos).reshape(-1, 2)
        rescuer_scatter.set_offsets(rescuer_pos)
        rescuer_scatter.set_facecolors(rescuer_colors)  # 保持颜色不变
        ...
        for i, r in enumerate(env.rescuers):
            ...
            if r.path:
                path_nodes = [r._get_current_node()] + r.path
                x_coords = [env.graph.nodes[n].x for n in path_nodes]
                y_coords = [env.graph.nodes[n].y for n in path_nodes]
                path_lines[i].set_data(x_coords, y_coords)
            else:
                path_lines[i].set_data([], [])
        ...
        task_pos = np.array([[env.graph.nodes[t.node_id].x,
                             ...
                             for t in active_tasks]) if active_tasks else np.empty((0, 2))
        ...
        # 清除旧标签
        for txt in ax_main.texts[2:]:  # 保留status_text和time_text
            txt.remove()

        # 添加新标签
        for t in active_tasks:
            node = env.graph.nodes[t.node_id]
            label = f"T{t.task_id}\nV:{t.survivors_not_rescued}\nDL:{t.deadline - env.current_time}"
            ax_main.text(node.x + 0.5, node.y - 0.8, label,
                         fontsize=6, color='darkred',
                         bbox=dict(facecolor='wheat', alpha=0.7, edgecolor='grey', boxstyle='round'))
        ...
        # 在update函数中更新状态文本：
        current_rescuers = len(env.rescuers)
        current_tasks = len([t for t in env.tasks if t.rescued_victim < t.victim])
        completion_rate = (current_rescued / env.total_victims * 100) if env.total_victims > 0 else 0

        status_text.set_text(
            f"Rescuers: {current_rescuers}\n"
            f"Tasks: {current_tasks}\n"
            f"Rescued: {current_rescued}/{env.total_victims}\n"
            f"({completion_rate:.1f}%)"
        )

        # 根据进度改变文字颜色（可选）
        status_color = 'dimgrey'  # 默认颜色
        if completion_rate > 90:
            status_color = '#228B22'  # 森林绿
        elif completion_rate > 50:
            status_color = '#DAA520'  # 金色
        ...
        progress_line2.set_data(rescue_data['time'], rescue_data['active_tasks'])
        progress_line3.set_data(rescue_data['time'], rescue_data['idle_rescuers'])
        ax_progress_right.relim()
        ax_progress_right.autoscale_view()

        # === 标题更新 ===
        ax_main.set_title(
            "Real-time Rescue Simulation\n"
            "Node Size: Rescue Capacity | Dashed Lines: Planned Paths",
            fontsize=10,
            pad=12
        )
        ...
        ax_progress.set_title(
            "Rescue Progress Monitoring\n"
            "Left: Victims | Right: Tasks/Rescuers",
            fontsize=10,
            pad=12
        )

        time_text.set_text(f"Time: {env.current_time}")  # 直接使用环境当前时间

        return (rescuer_scatter, task_scatter, progress_line, status_text, time_text)

    # ================== 图例配置 ==================
    legend_elements = [
        Line2D([0], [0],
               marker='o',
               color=rescuer_colors[i],
               label=f'Rescuer #{i}',
               markersize=10,
               linestyle='',
               markerfacecolor=rescuer_colors[i])
        for i in range(n_rescuers)
    ]
    legend_elements += [
        Line2D([0], [0], marker='s', color='w', label='Active Task',
               markerfacecolor='gold', markersize=10),
        Line2D([0], [0], color='gray', linestyle='--', label='Rescue Path',
               linewidth=2)
    ]
    ...
    ax_main.legend(
        ...
        bbox_to_anchor=(0.98, 0.05),  # 调整锚点位置
        ncol=1,  # 单列显示
        fontsize=8,
        title="Agents Legend",  # 修改图例标题
        title_fontsize=9,
        framealpha=0.8
    )
