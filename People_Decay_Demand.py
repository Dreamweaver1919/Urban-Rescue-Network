...
def Visualize(env):
    ...
    ax1.legend(handles=legend_elements, bbox_to_anchor=(0, 1), loc='upper left', fontsize=8)
    ...
    def update(frame, env, rescuer_scatter, task_scatter, status_text, time_text, progress_line, total_target_line):
        ...
        # -------------------- 获取救援人员数据（包含能力） --------------------
        rescuers_x = [r.x for r in env.rescuers]
        rescuers_y = [r.y for r in env.rescuers]
        rescuer_colors = ['red' if r.busy else 'green' for r in env.rescuers]
        # 能力映射为标记大小（假设能力范围1-3，映射为10-30像素）
        rescuer_sizes = [10 * r.efficiency * 2 for r in env.rescuers]  # 能力1:20, 能力2:40, 能力3:60

        # -------------------- 获取任务数据（保持不变） --------------------
        tasks_x, tasks_y, task_colors, task_sizes = [], [], [], []
        for task in env.tasks:
            if env.current_time > task.deadline:
                task_colors.append('gray')
            elif task.rescued_victim >= task.initial_victim:
                task_colors.append('blue')
            else:
                task_colors.append('orange')
            tasks_x.append(task.x)
            tasks_y.append(task.y)
            task_sizes.append(50 + task.survivors_not_rescued * 2)  # 任务大小仍由被困人数决定

        # -------------------- 更新救援人员散点图（新增size参数） --------------------
        rescuer_scatter.set_offsets(np.column_stack([rescuers_x, rescuers_y]))
        rescuer_scatter.set_color(rescuer_colors)
        rescuer_scatter.set_sizes(rescuer_sizes)  # 设置能力对应的大小

        task_scatter.set_offsets(np.column_stack([tasks_x, tasks_y]))
        task_scatter.set_color(task_colors)
        task_scatter.set_sizes(task_sizes)
    ...
    ani.save('rescue_simulation1.gif', writer='pillow', fps=5, dpi=100)
